...
export class CouponManagerService {
  ...
  aplicarCupon(codigo: string, total: number, callback: (ok: boolean, descuento: number) => void): void {
    ...
    this.couponService.validarCodigo(codigo).subscribe({
      ...
      error: (err) => {
        this.limpiar();
        this.error = err.error?.error || 'Error al validar el cupón.';
        callback(false, 0);
      }
    });
  }
  ...
  /*calcularTotalConDescuento(): number {
    if (!this.cuponAplicado) return this.totalOriginal;
    return this.totalOriginal - (this.totalOriginal * this.cuponAplicado.porcentaje_descuento / 100);
  }*/
