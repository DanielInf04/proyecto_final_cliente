...
export class CheckoutComponent implements OnInit, OnDestroy {
    ...
    this.locationService.getProvincias().subscribe({
      next: (res) => {
        ...
        if (this.authService.isLoggedIn()) {
          ...
          // Obtenemos la direccion guardada
  ...
  finalizarCompra(paypalDetails: any = null) {
    ...
    const payload: any = {
      ...
      cupon: this.cuponAplicado ? { id: this.cuponAplicado.id } : undefined,
    };

    // Si ya tenemos detalles de PayPal, es porque el botón ya fue aprobado
    if (paypalDetails) {
      ...
      this.checkoutPagoService.procesarCheckout(payload).subscribe({
        ...
        error: (err) => {
          ...
          if (err.status === 400 && err.error?.error === 'stock_insuficiente') {
            const errores = err.error.productos || [];
          ...
          if (err.status === 422 && err.error?.message) {
            ...
            this.toastService.showToast('No se pudo completar el pedido.', 'danger');
          }
        }
      });

    } else {
      // 🟡 Primera validación: validar datos antes de mostrar PayPal
      this.checkoutPagoService.validarDatos(payload).subscribe({
        next: (res) => {
          // Si es tarjeta, podrías ir directo al proceso final (si lo tuvieras implementado)
          if (this.myForm.value.metodo_pago === 'paypal') {
            this.mostrarPaypalButton = true;
          }
        },
        error: (err) => {
          console.error('Error de validación del backend:', err);
          ...
          if (err.status === 422 && err.error?.message) {
            ...
            this.toastService.showToast('Datos inválidos o incompletos. Revisa el formulario.', 'danger');
          }
        }
      });
    }
